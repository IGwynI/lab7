...
public static class CollectionTasks
{
    ...
    public static void AnalyzeFilmViewers(string[] films, HashSet<string>[] viewers)
    {
        ...
        List<string> allWatched = new List<string>();
        List<string> someWatched = new List<string>();
        List<string> noneWatched = new List<string>();
        ...
        for (int f = 0; f < films.Length; f++)
        {
            ...
            {
                noneWatched.Add(film);
            }
            else
            {
                someWatched.Add(film);
            }
        }
        ...
        {
            Console.WriteLine(string.Join(", ", allWatched));
        }
        ...
        {
            Console.WriteLine(string.Join(", ", someWatched));
        }
        ...
        {
            Console.WriteLine(string.Join(", ", noneWatched));
        }
    }
    ...
    public static double AverageEmployeesPerDepartment(string filePath)
    {
        ...
        using (StreamReader reader = new StreamReader(filePath))
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (lineNumber == 1)
                {
                    if (!int.TryParse(line.Trim(), out totalEmployeesExpected) || totalEmployeesExpected < 0)
                    {
                        throw new FormatException("Первая строка должна содержать неотрицательное целое число (количество сотрудников).");
                    }
                    skipLine = true;
                }
                ...
                if (!skipLine)
                {
                    ...
                    if (lastSpaceIndex <= 0)
                    {
                        throw new FormatException($"Строка {lineNumber} имеет неверный формат: {line}");
                    }
                    ...
                    {
                        throw new FormatException($"Некорректный номер телефона в строке {lineNumber}: {phone}");
                    }
        ...
        {
            throw new InvalidDataException($"Количество обработанных сотрудников ({processedEmployees}) не совпадает с заявленным в файле ({totalEmployeesExpected}).");
        }
